...
def get_opciones():
    ...
    lineas = db.lineas.distinct("linea_id")
    
    # Obtener nombres de estudios
    estudios = db.campus.distinct("estudios.nombre")
    
    # Obtener nombres de campus
    campus = [c["nombre"] for c in db.campus.find({}, {"nombre": 1})]
    
    return jsonify({
        "lineas": sorted(lineas),
        "estudios": sorted(estudios),
        "campus": sorted(campus)
    })
